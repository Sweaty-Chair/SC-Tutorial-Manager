...
public class TutorialShowPanelTaskEditor: TutorialTaskEditor
{
	...
	protected virtual void OnHandSettingGUI()
	{
		...
		if (showHandSetting) {
			...
			if (showHand != _tspt.showHand) {
				Undo.RegisterUndo(_tspt, "Reassign Show Hand");
			...
			if (_tspt.showHand) {
				...
				if (handLocalPosition != _tspt.handLocalPosition) {
					Undo.RegisterUndo(_tspt, "Reassign Hand Local Position");
				...
				if (handLocalRotation != _tspt.handLocalRotation) {
					Undo.RegisterUndo(_tspt, "Reassign Hand Local Rotation");
				...
				if (handLocalScale != _tspt.handLocalScale) {
					Undo.RegisterUndo(_tspt, "Reassign Hand Local Scale");
	...
	protected virtual void OnTextSettingGUI()
	{
		...
		if (showTextSetting) {
			...
			if (showText == string.IsNullOrEmpty(_tspt.text)) {
				Undo.RegisterUndo(_tspt, "Reassign Show Text");
			...
			if (showText) {
				...
				if (textLocalPosition != _tspt.textLocalPosition) {
					Undo.RegisterUndo(_tspt, "Reassign Text Local Position");
				...
				if (textLocalRotation != _tspt.textLocalRotation) {
					Undo.RegisterUndo(_tspt, "Reassign Text Local Rotation");
				...
				if (textSize != _tspt.textSize) {
					Undo.RegisterUndo(_tspt, "Reassign Text Size");
				...
				if (text != _tspt.text) {
					Undo.RegisterUndo(_tspt, "Reassign Text String");
					_tspt.text = text;
					UpdatePreview();
				}
				...
				if (textColor != _tspt.textColor) {
					Undo.RegisterUndo(_tspt, "Reassign Text Color");
				...
				if (alignment != _tspt.alignment) {
					Undo.RegisterUndo(_tspt, "Reassign Text Alignment");
	...
	protected virtual void OnBackgroundMaskSettingGUI()
	{
		...
		if (showBackgroundSetting) {
			...
			if (showBackgroundMask != _tspt.showBackgroundMask) {
				Undo.RegisterUndo(_tspt, "Reassign Show Background Mask");
			...
			if (showBackgroundMask) {
				...
				if (alpha != _tspt.alpha) {
					Undo.RegisterUndo(_tspt, "Reassign Background Mask Alpha");
					_tspt.alpha = alpha;
					UpdatePreview();
				}
				...
				if (clickToComplete != _tspt.clickToComplete) {
					Undo.RegisterUndo(_tspt, "Reassign Show Background Click To Complete");
                ...
                if (clickToComplete)
                {
                    ...
                    {
                        Undo.RegisterUndo(_tspt, "Reassign Show Background Time Before Clickable");
	...
	{
		bool doNotResetPanelOnComplete = EditorGUILayout.Toggle(new GUIContent("Do Not Reset Panel On Complete", "Do not reset the panel after tutorial step complete. This is mainly used when next tutorial step also use the tutorial panel, so avoid flicker of the tutorial panel.") , _tspt.doNotResetPanelOnComplete);
		if (doNotResetPanelOnComplete != _tspt.doNotResetPanelOnComplete) {
			Undo.RegisterUndo(_tspt, "Reassign Do Not Reset Panel On Complete");
			_tspt.doNotResetPanelOnComplete = doNotResetPanelOnComplete;
		}
	}
	...
	protected override bool isPreviewShown {
		get {
			return _tspt.tutorialPanel != null;
		}
	}
