	...
	public class TutorialDragTaskEditor : TutorialHighlightTaskEditor
	{
		...
		protected override void OnOtherSettingGUI()
		{
			...
			if (constraintDirection != _tod.constraintDirection) {
				EditorUtility.SetDirty(_tod);
				Undo.RegisterCompleteObjectUndo(_tod, "Reassign Constraint Direction");
			...
			if (constraintDirection) {
				...
				if (dragDirectionAngle != _tod.dragDirectionAngle) {
					EditorUtility.SetDirty(_tod);
					Undo.RegisterCompleteObjectUndo(_tod, "Reassign Drag Direction Angle");
				...
				if (inverseDirection != _tod.inverseDirection) {
					EditorUtility.SetDirty(_tod);
					Undo.RegisterCompleteObjectUndo(_tod, "Reassign Inverse Direction");
			...
			if (dragDistance != _tod.dragDistance) {
				EditorUtility.SetDirty(_tod);
				Undo.RegisterCompleteObjectUndo(_tod, "Reassign Drag Distance");
			...
			if (scaleDistanceWithScreenWidth != _tod.scaleDistanceWithScreenWidth) {
				EditorUtility.SetDirty(_tod);
				Undo.RegisterCompleteObjectUndo(_tod, "Reassign Scale Distance With Screen Width");
			...
			if (accumulateDistance != _tod.accumulateDistance) {
				EditorUtility.SetDirty(_tod);
				Undo.RegisterCompleteObjectUndo(_tod, "Reassign Accumulate Distance");
